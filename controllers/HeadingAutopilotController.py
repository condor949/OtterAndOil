...
class HeadingAutopilotController(BaseController):
    """
    Controller that uses heading autopilot for each vehicle.
    
    The heading autopilot uses a PID controller based on pole placement
    to control the vehicle's heading angle.
    """
    name = 'heading_autopilot'
    abbreviation = 'HA'
    ...
    def __init__(self, vehicles, sim_time: int, sample_time: float, space,
                 ...
                 smoothing: float, desired_heading_deg=0, tau_X=120, 
                 isolines=10, FPS=30, **kwargs):
        """
        Initialize the heading autopilot controller.
        ...
        Args:
            ...
            desired_heading_deg: Desired heading angle in degrees
            tau_X: Surge force (N) - forward thrust
        """
        super().__init__(vehicles, sim_time, sample_time, space, eps, e_max_cap,
                        dynamic_error_max, smoothing)
        
        # Настройки автопилота курса
        ...
        # Инициализация состояния автопилота для каждого транспортного средства
        for vehicle in self.vehicles:
            if hasattr(vehicle, 'headingAutopilot'):
                # Установка желаемого курса
                vehicle.ref = desired_heading_deg
                vehicle.tauX = tau_X
                logger.debug(
                    "Configured heading autopilot for vehicle %d: psi_d=%.1f deg, tau_X=%.1f N",
                ...
                logger.warning(
                    "Vehicle %s does not support headingAutopilot method",
                    vehicle
                )
    
    def generate_control(self, positions, step, relative_velocities):
        """
        Генерирует управляющие сигналы используя headingAutopilot для каждого транспортного средства.
        ...
        for i, vehicle in enumerate(self.vehicles):
            ...
            if hasattr(vehicle, 'headingAutopilot'):
                # Используем headingAutopilot для управления курсом
                u_control = vehicle.headingAutopilot(eta, nu, self.sample_time)
            else:
                # Fallback: нулевое управление
                logger.warning(
                    "Vehicle %s does not support headingAutopilot, using zero control",
                    vehicle
                )
    ...
    def __str__(self):
        return (f'---controller--------------------------------------------------------------------------\n'
                f'Heading Autopilot Controller\n'
                ...
                f'Desired heading: {self.desired_heading_deg} deg\n'
                f'Surge force (tau_X): {self.tau_X} N')
